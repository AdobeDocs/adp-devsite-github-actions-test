...
- Name: content.spec.js >> content: /firefly-services/docs/photoshop/guides/photoshop-v2-beta/v1-to-v2/layer-operations-adjustments
- Location: test/content.spec.js:92:3

# Error details

```
Error: Content issues on stage for /firefly-services/docs/photoshop/guides/photoshop-v2-beta/v1-to-v2/layer-operations-adjustments
...
- Array []
+ Array [
+   "LINKS: 1 broken anchor(s) — #3-adjustments-structure--required-type-in-v2",
+ ]
```
...
```ts
  23  | 
  24  |     // --- H1 (warning only — recommended for LLM readability) ---
  ...
  45  |     const excludedBlocks = ['site-wide-banner', 'contributors', 'discover', 'redoclyapiblock', 'side-nav', 'breadcrumbs', 'embed', 'next-prev', 'onthispage', 'fragment', 'horizontalline'];
  ...
  99  | 
  100 |     // Load prod to filter out pre-existing issues already broken on prod
  101 |     const prodRes = await page.goto(`${PROD_BASE}${path}`);
  102 |     let prodIssues = new Set();
  103 |     if (prodRes?.status() === 200) {
  104 |       await page.waitForLoadState('load');
  105 |       const { issues: pi } = await extractIssues(page);
  106 |       prodIssues = new Set(pi);
  107 |     }
  108 | 
  109 |     // Only flag issues that are new on stage (not already broken on prod)
  110 |     const issues = stageIssues.filter(i => !prodIssues.has(i));
  111 | 
  112 |     if (issues.length || warnings.length) {
  113 |       const lines = [`PAGE: ${path}`, ''];
  114 |       if (issues.length) lines.push(...issues.map(i => `  ❌  ${i}`));
  115 |       if (warnings.length) lines.push('', ...warnings.map(w => `  💡  ${w}`));
  116 |       await testInfo.attach('content-report', { body: lines.join('\n'), contentType: 'text/plain' });
  117 |     }
  118 | 
  119 |     for (const w of warnings) {
  120 |       testInfo.annotations.push({ type: 'warning', description: w });
  121 |     }
  122 | 
> 123 |     expect(issues, `Content issues on stage for ${path}`).toEqual([]);
      |                                                           ^ Error: Content issues on stage for /firefly-services/docs/photoshop/guides/photoshop-v2-beta/v1-to-v2/layer-operations-adjustments
  124 |   });
  125 | }
  126 | 
